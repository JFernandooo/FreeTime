...
| Versão | Destaques |
|--------|-----------|
| v2.1.12 | Corrigido o botão "Concluir" na notificação (não funcionava com a app fechada); lembretes com hora ficam "Vencidos" a partir dela; corrigido o cálculo de "faltam X dias"; novo ícone na barra de notificações |
| v2.1.11 | Concluir um lembrete, tomar um medicamento ou arquivar uma lista de compras passam também a dar experiência (XP), tal como já acontecia com Tarefas e Hábitos |
| v2.1.10 | Notificações com hora definida mostram a hora; corrigido bug grave em que editar um lembrete criava uma conclusão falsa no histórico; notificações do Modo Insistente ganharam os botões "Concluir"/"Adiar 1h" |
| v2.1.9 | Corrigida a causa real de "não foi possível agendar a notificação" em builds de release (regras de R8 em falta); a mensagem também deixa de apontar sempre para a permissão de alarme exato quando não era essa a causa |
| v2.1.8 | (substituída pela v2.1.9 — mesma correção, republicada com novo número de versão) |
